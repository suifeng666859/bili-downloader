...
- **可选携带登录 Cookie（SESSDATA）** —— 填了才能拿到 1080P；不填服务端最高只发 480P，且码率被压得很低
...
## 想下 1080P？填一下登录 Cookie

B站对**未登录**的请求只发 480P，而且码率会被压到 200~300 kbps（实测一个视频只有 185 kbps），
所以看起来就是糊的。这是服务端限制，不是程序的问题。想拿 1080P 需要带上登录态：

1. 用浏览器（Edge / Chrome）打开 bilibili.com，确认已登录
2. 按 `F12` → 顶部切到 **应用程序 / Application** → 左侧 **Cookies** → 展开 `https://www.bilibili.com`
...
- **1080P 及以上需要登录**。不带 Cookie 时服务端最高只给 480P，且码率被压到 200~300 kbps（看着就是糊的），这是 B 站的服务端限制，不是代码问题。填「登录 Cookie」即可解决，见上方章节。
